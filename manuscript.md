...
- 收获·工程能力：C++/Java 混合栈开发，10+ 表达式提 PR 走通全链路
- 收获·方法论：vanilla 二分法、只读旁路、纯净原则审计、声明式编排
- 收获·认知升级：工具是行为约束，隐式交互脆弱，看起来完整≠真的能用
- 不足·底层深度：向量化内核与 LLVM codegen 实现了解不够
- 不足·实战验证：skill 包零真实场景验证，是最佳实践陈述
- 不足·时间分配：8 月切 Agent 后 OmniStream 深度推进受限
...
工程能力与方法论显著成长，底层深度与实战验证是主要短板。

### Notes:

两个月实习在工程能力与方法论上显著成长。工程能力：C++/Java 混合栈开发，OmniStream 10+ 表达式提 PR 走通设计→实现→审计→修复全链路，Agent 工程 4 角色流水线落地加公式原生插入。

方法论沉淀 15 条，核心几条：vanilla 对照组二分法做 bug 归因（先问"这是谁的 bug"）；只读旁路可观察性（观测不能改变被观测对象行为，trace 落 workspace 不落 skill 目录）；纯净原则审计清单（溯源残留/装饰冗余/死内容/doc-impl 裂隙/语言平台残留五类 16 项）；声明式编排加动态生成代码（workflow.md 声明流程框架生成 workflow.py 执行）；降级设计（首要路径错误返回下一降级路径）；允许 agent 犯错及时提示纠正。

认知升级：工具是行为约束不只声明能力（skill 是"我只能做什么"）；隐式交互是脆弱的（HITL 靠模型自觉会被跳过，显式 ask_user 才是强约束）；思维链过长等于执行风险；看起来完整不等于真的能用（skill 包零实战验证是最佳实践陈述非工作流提炼）；人工介入审计是质量闸门。

不足在底层深度不够，对 OmniOperator 最底层向量化内核与 LLVM codegen 实现了解不够清楚，从能用走向吃透还需深入。skill 包最大缺口是零实战验证。8 月切入 Agent 项目后 OmniStream 深度推进受限，时间分配需优化。
