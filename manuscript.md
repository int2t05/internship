...
学习与工作交织推进。个人相关打基础：大数据基础学习建立流处理全貌认知，Agent生态调研理清skill驱动机制。工作相关出产出：OmniStream表达式开发10+表达式提PR，PPT-Agentskill开发4角色流水线落地加公式原生插入WPS验证。两者相互支撑。
...
Java适配层：拦执行计划、判断Native化、JNI初始化、不支持回退
C++核心层：算子、状态、数据流、连接器全在C++闭环
零侵入接入，只改两个配置文件，不改Flink内核代码
...
选型依据：固定流水线+断点续跑→编排式；一人用+流程灵活→单agent
...
基础学习：PPTAgent框架解析、九问skill机制、swarmflow原语
知识拓展：4种组装方式选型、公式插入8项目对比、样式模板调研
工作指南：PresentBench 30条检查项、三agent产物设计原则
工程实践：4分支审计+纯净性总结，50+commit可追溯
21篇约34万字，5个开源项目横向调研
...
方法论沉淀：vanilla二分法、只读旁路可观察性、纯净原则审计
不足：
底层向量化内核与编译器实现了解不够清楚
