...
两个月实习聚焦华为 Omni 生态两个项目，从基础学习切入，逐步深入到开发与工程化。

**教育经历**：2023.09 — 2027.06　华南理工大学　计算机科学与技术

**入职华为**：2026.07.01，SAIE业务域（Omni 生态大数据方向）
...
timeline
    title 实习主线
    7月上中旬 : 大数据基础学习 : OmniStream 表达式开发
...
<tr><td rowspan="3">个人相关</td><td>1、大数据基础学习：Flink/Spark 流批架构、状态管理与容错、鲲鹏 920 ARM 生态、电商日志全流程</td><td>建立流处理全貌认知 理解 Omni 生态定位 画模块关系图找准切入链路</td></tr>
<tr><td>2、Agent 生态调研：九问 agent-core/swarmflow 原语机制、27 个开源 PPT 方案横向调研</td><td>理清 skill→workflow.md→workflow.py 执行链路 输出调研报告解析到源码层面</td></tr>
<tr><td>3、开发工具链建设：6 个表达式开发 skill + 构建优化 skill + 审计 skill</td><td>固化"构建→测试→审计"开发流 重复动作一条命令化 培养工具沉淀意识</td></tr>
<tr><td rowspan="4">工作相关</td><td>1、OmniStream 表达式开发：8 类 SQL 表达式原生化（ISNULL/LEFT-RIGHT/BETWEEN/NOT IN/EXISTS/SIMILAR TO/IFNULL 等）</td><td>10+ 表达式提 PR 走通"设计→实现→提 PR→审计→修复→补测试"全链路 编码能力提高</td></tr>
<tr><td>2、OmniStream bug 修复：filter 路径 BetweenExpr 崩溃、注册名错位、静默回退等</td><td>确立 vanilla 对照组二分法 问题归因有客观依据 培养根因排查能力</td></tr>
<tr><td>3、PPT-Agentskill 开发：researcher 部分、.trace/ 可观察性体系、公式原生插入方案</td><td>4 角色流水线落地 OMML 方案 26 公式 WPS 验证通过 掌握可编辑产物工程化</td></tr>
<tr><td>4、PPT-Agentskill 调优：HITL 转达显式化（PR#7）、场景路由+产物质量保障（PR#8）、纯净原则审计</td><td>建立脚本+LLM 双层质量保障体系 沉淀纯净原则审计清单 4 大类 16 项 掌握大重构工程组织</td></tr>
</table>
...
# Slide 7: OmniStream项目背景

OmniStream 是 openEuler 社区 + 鲲鹏 BoostKit 大数据的流处理加速项目，用原生代码重写 Flink 算子，从根上提升性能。
...
OmniStream 四招对症下药：原生代码重写算子消除虚拟机开销、向量化指令批量计算、整条算子链下沉原生减少跨语言调用、状态缓存降低磁盘 IO。
...
> 项目属 openEuler 社区 + 鲲鹏 BoostKit 大数据 OmniRuntime，适配 Flink 1.16.3，当前版本 1.3.0。
...
| 仓库 | 职责 | 技术栈 | 产物 |
| --- | --- | --- | --- |
| OmniStream | 原生运行时框架：Task/算子链/状态/连接器 | C++ + CMake | libtnel.so |
| OmniAdaptor | Flink 桥接：算子替换决策、执行计划注入 | Java | flink-tnel.jar |
| OmniOperator | 底层向量化内核：算子/表达式/150+函数 | C++ + Java JNI | 5×.so + 2×.jar |
...
让 Flink SQL 表达式走原生加速路径，是 OmniStream 侧的主要工作。
...
**方法**：把同一用例同时跑在原生 Flink 与 Omni 原生实现上对比：

- **投影路径**：原生 Flink 也崩，异常栈在 Calcite 空集处理，属 Flink 1.16.3 源码缺陷，与 Omni 无关 → 测试主动规避这类输入
- **过滤路径**：原生 Flink 正常而 Omni 崩，根因是过滤算子生成 BETWEEN 表达式时崩溃 → 本侧 bug，修复
...
**方法论**：原生也崩 = 上游问题（规避，测试不做 golden）；原生正常而 Omni 崩 = 本侧 bug（修复）。不把时间浪费在错误的目标上。
...
一体机办公 Agent 把大模型能力做成本地化、可编辑交付的办公智能体。两个关键约束决定后续选型：产物必须可编辑（中文办公刚需，走原生 PPT 路线而非图片型导出）、本地化部署（接一体机模型降本）。
...
PPTAgent v2 是当前学术 SOTA（评测均分 4.44，超越商业系统 Gamma 的 4.36）。架构为 Researcher + Presenter 双 Agent 共享观察空间，加环境接地反思（渲染成像素图反馈修正）：
...
本 skill 架构为 4 角色专门化流水线，脚本校验 + LLM 自审双层质量保障：
...
- 8 月切入 Agent 项目后 OmniStream 深度推进受限，时间分配需优化
